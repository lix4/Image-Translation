%% Initializing Detector DataSets
trainDdata = 'dataset/trDd.mat';
testDdata = 'dataset/teDd.mat';
validateDdata = 'dataset/vaDd.mat';

trainRdata = 'dataset/trRd.mat';
testRdata = 'dataset/teRd.mat';
...
if exist(trainDdata) == 2
    l = load(trainDdata);
    trainDImages = l.trainDImages;
else   
...
trainDImages = imageDatastore(...
    ...
    save(trainDdata,'trainDImages');
end




if exist(validateDdata) == 2
    l = load(validateDdata);
    validateDImages = l.validateDImages;
else   
...
validateDImages = imageDatastore(...
    ...
    'IncludeSubfolders',true, ...
    'LabelSource', 'foldernames');
    save(validateDdata,'validateDImages');
end

if exist(testDdata) == 2
    l = load(testDdata);
    tesDImages = l.testDImages;
else   
subdir = [rootdir 'test'];
...
if exist(trainRdata) == 2
    l = load(trainRdata);
    trainRImages = l.trainRImages;
else  
subdir = [rootdir 'train'];
...
trainRImages = imageDatastore(...
    ...
    'IncludeSubfolders',true, ...
    'LabelSource', 'foldernames');
    save(trainRdata,'trainRImages');
end

if exist(testRdata) == 2
    l = load(testRdata);
    testRImages = l.testRImages;
else  
subdir = [rootdir 'test'];
