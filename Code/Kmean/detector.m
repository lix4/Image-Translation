...
close all
clc
if ~exist('first_layer_centroids_detector_48.mat', 'file')
...
trainImages = imageDatastore(...
    ...
    N = size(trainImages.Files,1); % number of training examples
    ...
    while(cnt ~= N * n_pat )
        ...
        img = readimage(trainImages,k);
        img = img(ypos:ypos+sz-1,xpos:xpos+sz-1);
        img = img(:);
        patches(cnt,:) = img';
        cnt = cnt + 1;
    end
    ...
    save('first_layer_centroids_detector_48.mat','D','M','P', '-v7.3');
else
    load first_layer_centroids_detector_48.mat;
end

trainDataFile='detectorTrain.mat';
cvDataFile='detectorCV.mat';
maxIter=150;
